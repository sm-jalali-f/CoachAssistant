...
class JalaliCalendarProviderTest {
    ...
    fun test_getCurrentDate_returns_correct_date() {
        ...
        val actual = CalendarDate(1404, 8, 25, WeekDay.Sunday.getPersianName(), LocalDate.now())
        assertEquals(expected, actual)
    }

    @Test
    fun test_getDates_returns_correct_dates() {
        val from = CalendarDate(1404, 6, 25, WeekDay.Tuesday.getPersianName(), LocalDate.of(2025, 9, 16))
        val to = CalendarDate(1404, 7, 2, WeekDay.Wednesday.getPersianName(), LocalDate.of(2025, 9, 24))
        val dates = provider.getDates(from, to)
        assertEquals(9, dates.size)
        assertEquals(from, dates.first())
        assertEquals(
            CalendarDate(1404, 6, 31, WeekDay.Monday.getPersianName(), LocalDate.of(2025, 9, 22)),
            dates[6],
        )
