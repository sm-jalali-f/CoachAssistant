...
interface CalendarProvider {
    ...
    /**
     * Returns a list of `AppDate` objects for every calendar day in the inclusive range from `from`
     * to `to`.
     *
     ...
     * @return A `List<AppDate>` containing one entry per day from `from` to `to` in ascending
     *   order.
     * @throws IllegalArgumentException If `from` or `to` is not a valid date in `yyyy-MM-dd`
     *   format.
     *
